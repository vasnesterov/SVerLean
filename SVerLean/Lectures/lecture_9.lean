...
Другими словами, мы ищем *неподвижную точку*.

Большая часть этой лекции посвящена построению оператора наименьшей неподвижной
точки `lfp`, который позволит нам определить случай `while`:
...
В общем случае наименьшая и наибольшая неподвижные точки могут различаться:

    `X = X`

Здесь наименьшая неподвижная точка — это `fun _ ↦ False`, а наибольшая неподвижная точка —
`fun _ ↦ True`. По соглашению `False < True`, и поэтому
`(fun _ ↦ False) < (fun _ ↦ True)`. Аналогично `∅ < Set.univ`.
...
2. У любого подмножества `S` есть инфимум `⨅ S`,
  т.е. такой элемент `x` что `x ≤ a` для всех `a ∈ S`.
3. Аналогично у каждого подмножества `S` есть супремум `⨆ S`,
  т.е. такой элемент `x` что `a ≤ x` для всех `a ∈ S`.
